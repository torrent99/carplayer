...
def fetch_lyrics(artist, title):
    ...
    try:
        url = 'http://ttlrcct2.qianqian.com/dll/lyricsvr.dll?sh?Artist={fred}&Title={frida}&Flags=0'.format(
                fred=EncodeArtTit(artist.replace(' ','').lower()), 
                frida=EncodeArtTit(title.replace(' ','').lower()), 
                )
